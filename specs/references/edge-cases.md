...
Edge cases are **not a separate system**. Every resolver uses the three components
that already exist in the platform — no new infrastructure required.

```
1. DETECT   — domain event handler calls PolicyEngine.evaluate()
              Detection thresholds live in the DB (tenant_policies), not in code.

2. APPROVE  — if verdict.triggered: WorkflowEngine.spawn("edge_case_approval", ...)
              Approval chain (single / dual / quorum) configured in workflow_definitions.

3. RESOLVE  — on approval, a named async resolver function executes the fix.
              AuditLedger.record() is called automatically by the workflow step.
              DomainEventBus.publish("edge_case.resolved", ...) closes the loop.
```

There is no separate `edge_cases` tracking table. The workflow instance in the
Workflow Engine is the record of the active case. The AuditLedger is the permanent
record of the outcome.

Schema changes within each entry (e.g. adding a `kyc_status` column to `applications`)
are additions to **existing module tables** — they are data model changes, not
edge case infrastructure.

```python
# The same four lines underpin every resolver in this document:

@domain_event_bus.subscribe("document.uploaded")       # 1. triggered by an event
async def check_identity_mismatch(event):
    verdict = await policy_engine.evaluate(            # 2. detection rule from DB
        "EC-ADM-01", event.payload, event.tenant_id
    )
    if verdict.triggered:
        await workflow_engine.spawn(                   # 3. approval via Workflow Engine
            "edge_case_approval",
            entity_id   = event.payload["application_id"],
            resolver    = "identity_mismatch_resolver",
            tier        = verdict.approval_tier,       # from policy DSL
            tenant_id   = event.tenant_id,
        )

async def identity_mismatch_resolver(context, tenant_id):   # 4. named resolver
    # execute the fix — AuditLedger recorded by workflow step
    ...
```

Detection thresholds, approval tiers, and SLA hours for each edge case below are
configured as `tenant_policies` DSL entries — not hardcoded in Python.
