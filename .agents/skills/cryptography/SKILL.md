---
name: cryptography
description: Comprehensive cryptography guidance covering encryption algorithms, password hashing, TLS configuration, key management, and post-quantum considerations. Use when implementing encryption, choosing hashing algorithms, configuring TLS/SSL, managing cryptographic keys, or reviewing cryptographic implementations. QUAICU kernel — use for the K·02 TrustLedger, an RFC 6962 Merkle transparency log — SHA-256 leaf/node hashing with 0x00/0x01 domain separation, inclusion and consistency proofs, Ed25519 signed tree heads via OpenBao, canonical JSON. Triggers — RFC 6962, TrustLedger, K·02, Merkle, inclusion proof, consistency proof, signed tree head, ledger seal.
allowed-tools: Read, Glob, Grep, Task
---

## ⚡ QUAICU Decision Contract — READ THIS FIRST (when used for the QUAICU kernel)

> Makes the QUAICU-specific crypto choices mechanical so a small/low-token model matches a top model at max effort.
> **For QUAICU work, this block overrides the general guidance below.** Missing rule → use the RFC exactly and stop.

### Invariants — never violated
- The TrustLedger (K·02) is an RFC 6962 transparency log. Implement EXACTLY: leaf = `SHA256(0x00 ‖ entry)`, node = `SHA256(0x01 ‖ left ‖ right)`. Never omit domain separation; never invent a proof shape.
- Hash only CANONICAL JSON (sorted keys, fixed separators, UTF-8). Same logical entry → identical bytes → identical hash.
- Sign tree heads (STH) with Ed25519; the signing key lives in OpenBao. NEVER hardcode or commit keys.
- Use a vetted library (`cryptography`, `hashlib`). NEVER hand-roll a hash, MAC, or signature.
- Secrets manager is OpenBao (MPL-2.0). NEVER `hvac`/HashiCorp Vault (BSL).

### Decision table
| Need | Use |
|---|---|
| Ledger leaf / interior hash | SHA-256 with 0x00 / 0x01 prefix |
| Sign/verify a tree head | Ed25519 (key from OpenBao) |
| Serialize before hashing | canonical JSON (key-sorted) |
| Password/credential hashing (host side) | Argon2id — never for ledger entries |
| Randomness for crypto | `secrets` / CSPRNG — never `random` |

### Tie-break rules
- Deviate from RFC 6962 for convenience? → never. The regulator verifies against the RFC.
- Where do signing keys live? → OpenBao, injected at runtime; never in code/env files.

### Self-check
- [ ] Leaf/node hashing uses 0x00/0x01 domain separation.
- [ ] Canonical JSON used for every hashed entry.
- [ ] STH signed with Ed25519; key from OpenBao; no committed keys.
- [ ] No hand-rolled primitives; no `hvac`/Vault.
