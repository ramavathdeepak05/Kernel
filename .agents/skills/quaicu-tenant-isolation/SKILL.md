...
description: |
  QUAICU multi-tenancy isolation enforcer — schema-per-tenant, per-tenant asyncpg connection pools,
  RLS as defense-in-depth on every kernel table, async tenant provisioning with job status tracking,
  schema version tracking, GDPR Article 20 tenant data export, tenant deletion with ledger archival,
  cross-shard query routing, tenants-per-DB scaling model, connection string rotation, full RLS
  policy for every kernel table, and batched migration rollout orchestration with per-tenant rollback.
  Use when building storage adapters, migrations, the control plane, or any code that touches data
  for multiple tenants. Enforces: Frozen Decision F-07 (per-tenant ledger tables always),
  schema-per-tenant default, no shared ledger table, RLS on every tenant table, tenant_id in
  ContextVar for RLS enforcement. Trigger keywords: tenant, schema_per_tenant, TenantId, tenant
  isolation, RLS, Row Level Security, current_tenant, ledger isolation, onboard_tenant, tenant
  schema, migration per tenant, tenant_id, connection pool, GDPR export, tenant deletion, shard,
  cross-shard, connection rotation, provisioning job, schema version, migration rollout.
---
