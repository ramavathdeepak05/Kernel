...
description: |
  QUAICU replay-safe design enforcer — full event store schema with aggregate_id and sequence,
  snapshot optimization (checkpoint every N events + replay from latest snapshot), streaming
  replay cursor for large ledgers, projection rebuild worker, conflict detection in counterfactual,
  signed point-in-time evidence pack for K·14, idempotent state reconstruction, and
  side-effect-free replay guarantee. Use when designing or reviewing any component that touches
  state, the ledger, K·12 rollback, K·13 sandbox, or K·14 point-in-time evidence. Enforces:
  Frozen Decision F-09, non-determinism recorded and reused (never recomputed), replay never
  causes external effects, event-sourced shape. Trigger keywords: replay, event_sourcing,
  state_reconstruction, counterfactual, audit_replay, side_effect_free, recorded_nondeterminism,
  record_activity, replay_safe, rollback, sandbox, ledger event log, point_in_time, projection,
  F-09, snapshot, replay cursor, streaming replay, evidence pack, conflict detection, idempotent.
---
