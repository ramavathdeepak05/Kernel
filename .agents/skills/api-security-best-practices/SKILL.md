...
description: "Implement secure API design patterns including authentication, authorization, input validation, rate limiting, and protection against common API vulnerabilities. QUAICU kernel — identity comes from the host via IdentityPort, tenant_id from the verified JWT claim only, fail-closed authz, no-bypass (every state change goes through evaluate+gate), secrets from OpenBao, every request audited. Triggers — QUAICU, IdentityPort, tenant_id JWT, no-bypass, fail-closed authz, RFC 7807, per-tenant rate limit."
risk: unknown
source: community
...
> Makes the QUAICU-specific API-security choices mechanical so a small/low-token model matches a top model at max effort.
> **For QUAICU work, this block overrides the general guidance below.**

### Invariants — never violated
- The kernel does NOT own authentication — it consumes a validated identity from the host (IdentityPort). Validate the JWT (signature, exp, audience) and take `tenant_id` from the verified claim, never from a body/header the client controls.
- Authorization is fail-closed: unresolved identity, expired token, or tenant-claim mismatch → reject (401/403). Never proceed as an anonymous or default actor.
- No-bypass: every state-changing request goes through propose→evaluate→gate. There is NO "admin" or "internal" path that skips evaluation.
- Secrets (JWT keys, signing keys) come from OpenBao at runtime. NEVER commit or env-bake secrets.
- Every request produces an audit-trail entry; a request that cannot be audited is denied.

### Decision table
| Situation | Do exactly this |
|---|---|
| tenant_id source | verified JWT claim only |
| Token invalid/expired | 401, fail-closed |
| Token tenant ≠ request tenant | 403 `TenantIsolationError` |
| Rate-limit / abuse | per-tenant limits; reject over budget |
| Error response shape | RFC 7807, no stack traces |

### Tie-break rules
- Trust a header/body tenant id when the JWT also carries one? → JWT claim wins; the other is ignored/rejected.
- Add a fast internal path that skips evaluation? → never; no-bypass is absolute.

### Self-check
- [ ] JWT fully validated; tenant_id from claim only.
- [ ] Authz fail-closed; tenant mismatch → 403.
- [ ] No bypass path around evaluate+gate.
- [ ] Secrets from OpenBao; every request audited.
