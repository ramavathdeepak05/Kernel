...
name: quaicu-testing
description: |
  QUAICU spec-driven testing strategy — full CI matrix (unit → conformance → property →
  integration → chaos), mutation testing setup (mutmut), coverage requirements per layer,
  contract tests between core and adapters, chaos testing patterns with random fault injection,
  performance baseline tests (p99 latency), golden case generation from YAML spec, test fixture
  factories for Action/Policy/LedgerEntry, parallelized test execution config, conformance suites,
  property-based invariant tests, fail-closed fault injection, tenant isolation adversarial tests,
  and replay side-effect tests. Use when writing tests for any QUAICU layer, setting up CI, or
  reviewing a PR for test coverage. Enforces: no layer is "done" without its correctness suite,
  property-based tests for all Core Invariants, fail-closed tested with injected faults, tenant
  isolation tested adversarially. Trigger keywords: conformance, property_based, hypothesis,
  fail_closed test, fault injection, golden case, acceptance suite, DoD, Definition of Done,
  invariant test, ledger integrity, policy determinism, replay test, pytest, pytest-asyncio,
  mutmut, mutation testing, coverage, contract test, chaos test, p99 latency, fixture factory,
  YAML spec, CI matrix, parallelized, pytest-xdist.
---
