...
3. **Your unit's skill.** Skills do **not** auto-load — no harness here injects `.agents/skills/`
   into your context. You **must Read it yourself**: look your directory up in **`SKILLS.md`** (the
   routing map at the workspace root) and open the listed `SKILL.md` in full *before writing any code*
   (e.g. `core/ledger/` → `quaicu-trust-ledger/SKILL.md` + `cryptography/SKILL.md`). The orchestrator
   names the exact path in your work brief; if it didn't, `SKILLS.md` has it. Each skill begins with a
...
the DoD it must meet, and **the exact path of the skill the implementer must Read first** (from `SKILLS.md`).

A dispatched sub-agent runs this loop:

**Definition of Ready (before starting — confirm all):**
- [ ] **The unit's skill (per `SKILLS.md`) has been Read in full** — its Decision Contract is open.
...
**Build:**
- **Read your skill first** (`SKILLS.md` → the mapped `SKILL.md`). No code before the contract is open.
- Implement only within the owned directory, against frozen interfaces.
- Follow that skill's decision contract mechanically.
...
The skills do **not** load themselves. Use **`SKILLS.md`** to find the skill for your directory and
**Read it before writing** — that one mechanical step is the difference between deterministic output and
an agent guessing. An hour of code written without the mapped skill open is an hour to throw away.
