...
class ExportEngine:
    ...
    def request_export(cls, org_id: str, req: ExportRequest, actor_id: str) -> dict:
        ...
        execute_transaction([(
            ...
            """,
            (job_id, org_id, req.report_type, req.format.value,
             json.dumps(req.filters), actor_id),
        )])

        try:
            from server.tasks.reporting import task_run_export
            task_run_export.delay(org_id, job_id)
        ...
        AuditLog.log(
            ...
            entity_type="export_job", entity_id=job_id, org_id=org_id,
            module="E11-S06",
    ...
    def _fetch_data(cls, org_id: str, report_type: str, filters: dict) -> list[dict]:
        ...
        if report_type == "attendance":
            ...
            return AcademicsReportService.attendance_summary(org_id, academic_year, semester)
        ...
        if report_type == "grades":
            ...
            return GradeService.get_student_grades(org_id, student_id, academic_year, semester)

        if report_type == "fee_collection":
            from server.finance.reports import FinanceReportService as FR
            return [FR.collection_summary(org_id, academic_year)]

        if report_type == "defaulters":
            from server.finance.reports import FinanceReportService as FR
            return FR.defaulters_list(org_id, academic_year)
