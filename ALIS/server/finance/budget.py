...
class BudgetService:
    ...
    def create_budget(
        ...
        execute_transaction([
            (
                ...
                """,
                (
                    budget_id, org_id, data["financial_year"],
                    data["department"], data.get("cost_center", ""),
                    data["allocated_amount"],
                    BudgetStatus.DRAFT.value, actor_id, now,
                ),
            )
        ], tenant_id=org_id)
    ...
    @classmethod
    def advance_status(
        cls, org_id: str, budget_id: str, target: str, actor_id: str,
    ) -> Dict[str, Any]:
        rows = execute_query(
            "SELECT status, department FROM budgets WHERE id = %s AND org_id = %s",
            (budget_id, org_id), tenant_id=org_id,
        )
        if not rows:
        ...
        if target_status == BudgetStatus.APPROVED:
            DomainEventBus.publish(DomainEvent(
                ...
                org_id=org_id,
                payload={"budget_id": budget_id, "department": rows[0]["department"]},
                actor_id=actor_id,
            ))

        AuditLog.log(
            action=AuditAction.UPDATE, actor_id=actor_id, actor_role="finance",
            entity_type="budget", entity_id=budget_id, tenant_id=org_id,
    ...
    def record_commitment(
        cls, org_id: str, budget_id: str, amount: float, reference: str, actor_id: str,
    ) -> Dict[str, Any]:
        ...
        available = float(budget["allocated_amount"]) - new_committed - float(budget["spent_amount"])
        if available < 0:
            raise ValueError(
    ...
    def check_variance(cls, org_id: str) -> List[Dict[str, Any]]:
        ...
        for r in rows:
            ...
            if util > 120:
                alerts.append({**dict(r), "severity": "CRITICAL", "message": f">20% overrun ({util:.0f}%)"})
            elif util > 110:
                alerts.append({**dict(r), "severity": "WARNING", "message": f">10% deviation ({util:.0f}%)"})
        return alerts
    ...
    def get_summary(cls, org_id: str, financial_year: str) -> List[Dict[str, Any]]:
        rows = execute_query(
            ...
            (org_id, financial_year), tenant_id=org_id,
        )
