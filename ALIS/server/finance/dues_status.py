...
class StudentDuesStatusService:
    ...
    async def get(cls, student_id: str, org_id: str) -> StudentDuesStatus:
        ...
        effective_outstanding = total_outstanding if not (exemption_active or promissory_cover) else 0
        dues_cleared = (
    ...
    async def assert_dues_cleared(cls, student_id: str, org_id: str) -> None:
        ...
        if not status.dues_cleared:
            breakdown = []
            if status.total_outstanding > 0 and not (status.exemption_active or status.promissory_cover_active):
