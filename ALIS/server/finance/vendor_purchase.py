...
class VendorService:
    ...
    @classmethod
    def register(cls, org_id: str, data: Dict[str, Any], actor_id: str) -> Dict[str, Any]:
        ...
        AuditLog.log(
            ...
            entity_type="vendor", entity_id=vendor_id, tenant_id=org_id,
    ...
    def approve(cls, org_id: str, vendor_id: str, actor_id: str) -> Dict[str, Any]:
        execute_transaction([
            ("UPDATE vendors SET status = %s, approved_by = %s, approved_at = NOW() "
             ...
             (VendorStatus.ACTIVE.value, actor_id, vendor_id, org_id, VendorStatus.PENDING.value))
        ], tenant_id=org_id)
        return {"id": vendor_id, "status": VendorStatus.ACTIVE.value}

    @classmethod
    def list_vendors(cls, org_id: str, status: Optional[str] = None) -> List[Dict[str, Any]]:
        if status:
            rows = execute_query(
...
class PurchaseOrderService:
    ...
    def create(cls, org_id: str, data: Dict[str, Any], actor_id: str) -> Dict[str, Any]:
        po_id = str(uuid4())
        po_number = f"PO-{datetime.now(timezone.utc).strftime('%Y')}-{po_id[:6].upper()}"
        now = datetime.now(timezone.utc)
        ...
        execute_transaction([
            (
                ...
                """,
                (
                    po_id, org_id, po_number, data["vendor_id"],
                    data.get("department", ""), data.get("description", ""),
                    _json_str(data.get("line_items", [])),
                    data.get("subtotal", 0), data.get("gst_amount", 0),
                    data.get("total_amount", 0), data.get("budget_head", ""),
                    POStatus.DRAFT.value, actor_id, now,
                ),
            )
        ], tenant_id=org_id)

        AuditLog.log(
            action=AuditAction.CREATE, actor_id=actor_id, actor_role="requester",
            entity_type="purchase_order", entity_id=po_id, tenant_id=org_id,
            metadata={"po_number": po_number, "vendor_id": data["vendor_id"]},
        )
        return {"id": po_id, "po_number": po_number, "status": POStatus.DRAFT.value}

    @classmethod
    def advance_status(cls, org_id: str, po_id: str, target: str, actor_id: str) -> Dict[str, Any]:
        """Move PO to next status (validates transition)."""
        rows = execute_query(
            "SELECT status FROM purchase_orders WHERE id = %s AND org_id = %s",
            (po_id, org_id), tenant_id=org_id,
        )
        if not rows:
        ...
        if target_status not in PO_TRANSITIONS.get(current, set()):
            raise ValueError(f"Invalid PO transition: {current.value} → {target_status.value}")
        ...
        AuditLog.log(
            action=AuditAction.UPDATE, actor_id=actor_id, actor_role="finance",
            entity_type="purchase_order", entity_id=po_id, tenant_id=org_id,
    ...
    @classmethod
    def three_way_match(
        cls, org_id: str, po_id: str, grn_amount: float, invoice_amount: float,
    ) -> Dict[str, Any]:
        """
    ...
    def compute_tds(cls, org_id: str, vendor_id: str, amount: float) -> Dict[str, Any]:
        ...
        rows = execute_query(
            ...
            (vendor_id, org_id), tenant_id=org_id,
        )
        if not rows:
    ...
    @classmethod
    def list_orders(cls, org_id: str, status: Optional[str] = None) -> List[Dict[str, Any]]:
        if status:
            rows = execute_query(
