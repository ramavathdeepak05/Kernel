...
async def get_digilocker_auth_url(
    request: Request, body: DigiLockerAuthRequest
) -> JSONResponse:
...
async def digilocker_callback(
    request: Request, body: DigiLockerCallbackRequest
) -> JSONResponse:
    from server.admissions.integrations.digilocker import DigiLockerClient
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"DigiLocker token exchange failed: {exc}"
        )
    ...
    return JSONResponse(
        {"success": True, "documents_pulled": len(docs), "documents": docs}
    )
...
async def import_nta_score(request: Request, body: NTAImportRequest) -> JSONResponse:
    ...
    if not client.is_enabled():
        raise HTTPException(
            status_code=503, detail="NTA integration not configured. Set NTA_API_KEY."
        )
    ...
    return JSONResponse(
        {
            ...
            "rank": sc.rank,
            "subject_scores": sc.subject_scores,
        }
    )
...
async def send_otp(request: Request, body: SMSOTPRequest) -> JSONResponse:
    ...
    result = client.send_otp(
        phone=body.phone, otp_length=body.otp_length, expiry_minutes=body.expiry_minutes
    )
    return JSONResponse(
        {
...
async def verify_otp(request: Request, body: SMSVerifyRequest) -> JSONResponse:
    ...
    result = client.verify_otp(
        phone=body.phone, otp=body.otp, request_id=body.request_id
    )
    return JSONResponse(
        {"is_valid": result.is_valid, "phone": result.phone, "error": result.error}
    )
...
async def lms_provision_student(request: Request, student_id: str) -> JSONResponse:
    ...
    if not service.is_enabled():
        raise HTTPException(
            status_code=503,
            detail="LMS integration not configured. Set LMS_BASE_URL and LMS_API_TOKEN.",
        )
    result = service.provision_student(
        org_id=org_id, student_id=student_id, actor_id=actor_id
    )
        ...
        return JSONResponse(
            {
                ...
                "last_synced_at": str(rows[0]["synced_at"])
                if rows[0]["synced_at"]
                else None,
            }
        )
...
async def get_presigned_upload_url(
    ...
    if url_obj is None:
        raise HTTPException(
            status_code=501,
            detail=f"Pre-signed URLs not supported for provider {client.provider!r}.",
        )
    return JSONResponse(
        {
            "url": url_obj.url,
            "storage_key": url_obj.storage_key,
            "expires_in_seconds": url_obj.expires_in_seconds,
