...
async def create_fee_structure(
    request: Request, body: FeeStructureCreate
) -> JSONResponse:
...
async def deactivate_fee_structure(request: Request, structure_id: str) -> JSONResponse:
    result = FeeStructureService.deactivate(
        _org(request), structure_id, _actor(request)
    )
...
async def bulk_generate_invoices(
    request: Request, body: InvoiceBulkCreate
) -> JSONResponse:
...
async def record_manual_payment(
    request: Request, body: ManualPaymentRecord
) -> JSONResponse:
...
async def student_scholarships(
    ...
    items = ScholarshipService.list_for_student(
...
async def default_risk(
    ...
    return JSONResponse(
        content=FeeDefaultAnalyticsService.predict_defaults(
            _org(request), academic_year
        )
    )
...
async def razorpay_webhook(request: Request) -> JSONResponse:
    ...
    try:
        ...
        logger.error(
            "razorpay_webhook: processing failed for event %s: %s", event_id, exc
        )
        execute_transaction(
            [
                (
...
async def raise_utr_dispute(request: Request, body: dict) -> JSONResponse:
    ...
    AuditLog.log(
        ...
        tenant_id=_org(request),
        metadata={"source": "raise_utr_dispute"},
    )
    return JSONResponse(
        status_code=201,
        content={
...
async def resolve_dispute(
    request: Request, dispute_id: str, body: dict
) -> JSONResponse:
    ...
    AuditLog.log(
        ...
        tenant_id=_org(request),
        metadata={"source": "resolve_dispute", "resolution": resolution},
    )
...
async def export_tally_xml(request: Request, body: dict) -> JSONResponse:
    ...
    xml_bytes = TallyExportService.export_tally_xml(
        org_id, from_date, to_date, _actor(request)
    )
    return FastAPIResponse(
        content=xml_bytes,
        media_type="application/xml",
        headers={
            "Content-Disposition": f"attachment; filename=tally_export_{from_date}_{to_date}.xml"
        },
    )
...
async def export_busy_csv(request: Request, body: dict) -> JSONResponse:
    ...
    from_date = body.get("from_date", "")
    to_date = body.get("to_date", "")
    csv_bytes = TallyExportService.export_busy_csv(
        org_id, from_date, to_date, _actor(request)
    )
    return FastAPIResponse(
        content=csv_bytes,
        media_type="text/csv",
        headers={
            "Content-Disposition": f"attachment; filename=busy_export_{from_date}_{to_date}.csv"
        },
    )
...
async def generate_irn(request: Request, invoice_id: str) -> JSONResponse:
    ...
    try:
        result = await EInvoiceService.generate_irn(
            invoice_id, _org(request), _actor(request)
        )
        return JSONResponse(content=result)
    except Exception as exc:
...
async def get_irn_status(request: Request, invoice_id: str) -> JSONResponse:
    ...
    return JSONResponse(
        content=result if result else {"irn": None, "status": "NOT_GENERATED"}
    )
