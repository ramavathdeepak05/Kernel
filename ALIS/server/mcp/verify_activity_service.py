...
def mock_execute_query(query, params):
    ...
    # INSERT COMMENT
    if "INSERT INTO COMMENTS" in query:
    ...
    # SELECT FEED (UNION)
    if "SELECT" in query and "ACTIVITY_FEED" in query and "COMMENTS" in query:
        ...
        for row in MOCK_ACTIVITY_TABLE:
            if (
                row["entity_type"] == target_entity_type
                and row["entity_id"] == target_entity_id
                and viewer_role in row["visible_to_roles"]
            ):
                results.append(
                    {
                        "type": "activity",
...
def run_verification():
    ...
    assert (
        "Internal Note: Documents look shaky. Verify with board."
        not in descriptions_std
    )
    ...
    assert (
        "This system is trash." not in descriptions_fac
    )  # Should be hidden for everyone
