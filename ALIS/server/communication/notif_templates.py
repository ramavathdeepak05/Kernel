...
class NotifTemplateService:
    ...
    def create(cls, org_id: str, req: TemplateCreate, actor_id: str) -> dict:
        ...
        execute_transaction([(
            ...
            """,
            (tid, org_id, req.key, req.name, req.subject, req.body, req.channel.value),
        )])

        AuditLog.log(
            action=AuditAction.CREATE, actor_id=actor_id, actor_type="human",
            entity_type="notification_template", entity_id=tid, org_id=org_id,
            module="E10-S01", metadata={"key": req.key, "channel": req.channel.value},
        )
    ...
    @classmethod
    def update(cls, org_id: str, template_id: str, req: TemplateUpdate, actor_id: str) -> dict:
        cls.get(org_id, template_id)
        fields, params = [], []
        if req.name is not None:
            fields.append("name = %s"); params.append(req.name)
        if req.subject is not None:
            fields.append("subject = %s"); params.append(req.subject)
        ...
        execute_transaction([(
            f"UPDATE notification_templates SET {', '.join(fields)} WHERE id = %s AND org_id = %s",
            params,
        )])
        return cls.get(org_id, template_id)

    @classmethod
    def resolve(cls, org_id: str, key: str, channel: str = "EMAIL") -> tuple[str, str] | None:
        """
