...
class DocumentTemplate(BaseModel):
    ...
    file_path: str = Field(
        ..., description="Path to template file relative to templates dir"
    )
    is_active: bool = Field(
        default=True, description="Whether template is currently active"
    )
...
class DocumentGenerationRequest(BaseModel):
    ...
    entity_id: str | None = Field(
        None, description="Related entity ID (student_id, etc.)"
    )
...
class GeneratedDocument(BaseModel):
    ...
    template_version: int = Field(
        ..., description="Template version at time of generation"
    )
