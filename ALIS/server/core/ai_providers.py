...
class AIProviderFactory:
    ...
    @staticmethod
    def _create_openai(
        config: AIProviderConfig, api_key: str, temp: float, max_tokens: int
    ) -> BaseLanguageModel:
    ...
    @staticmethod
    def _create_ollama(
        config: AIProviderConfig, _api_key: str, temp: float, _max_tokens: int
    ) -> BaseLanguageModel:
...
class TenantAIProviderService:
    ...
    @classmethod
    def get_active_provider(cls, tenant_id: str) -> AIProviderConfig | None:
    ...
    @classmethod
    def delete_provider(
        cls, tenant_id: str, config_id: str, actor_id: str = "system"
    ) -> bool:
    ...
    def _encrypt_key(cls, plaintext: str, tenant_id: str) -> str:
        ...
        key = base64.urlsafe_b64encode(
            hashlib.sha256((settings.app_secret_key + tenant_id).encode()).digest()
        )
    ...
    def _decrypt_key(cls, ciphertext: str, tenant_id: str) -> str:
        ...
        try:
            ...
            vault = get_vault_client()
            if vault._is_available():
                return vault.decrypt_data(
                    ciphertext, context=f"ai_provider:{tenant_id}"
                )
        except Exception:
            pass  # noqa: S110 — intentionally suppressed
        import base64
