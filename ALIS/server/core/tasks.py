...
from .notifications.service import get_dispatcher
...
class TaskService:
    ...
    def update_task(cls, task_id: str, update_in: TaskUpdate, updated_by: str) -> Task:
        ...
        if update_in.status is not None:
            ...
            if (
                update_in.status == TaskStatus.COMPLETED
    ...
    def get_tasks_for_user(cls, user_id: str, user_role: Role) -> list[Task]:
        ...
        return [
            ...
            and (
                ...
                or (t.assignee_role and t.assignee_role == user_role.value)
            )
        ]
    ...
    def _notify_assignment(cls, task: Task):
        ...
        if task.assignee_id:
            try:
                from server.db_service import execute_system_query_sync

                rows = execute_system_query_sync(
            ...
            try:
                ...
                logger.warning(
                    "TaskService: role-broadcast failed for role=%s: %s",
                    task.assignee_role,
                    exc,
                )
