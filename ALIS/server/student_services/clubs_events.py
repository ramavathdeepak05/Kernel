...
EVENT_TRANSITIONS = {
    ...
    EventStatus.VENUE_ALLOTTED: {EventStatus.BUDGET_SANCTIONED, EventStatus.IN_PROGRESS, EventStatus.CANCELLED},
...
class ClubService:
    ...
    @classmethod
    def propose_club(cls, org_id: str, data: Dict[str, Any], actor_id: str) -> Dict[str, Any]:
        club_id = str(uuid4())
        ...
        execute_transaction([
            (
                ...
                """,
                (club_id, org_id, data["name"], data.get("description", ""),
                 data.get("category", "GENERAL"), actor_id,
                 data.get("faculty_advisor_id"), ClubStatus.PROPOSED.value, now),
            )
        ], tenant_id=org_id)
        return {"id": club_id, "status": ClubStatus.PROPOSED.value}

    @classmethod
    def assign_advisor(cls, org_id: str, club_id: str, advisor_id: str, actor_id: str) -> Dict[str, Any]:
        cls._validate_transition(org_id, club_id, ClubStatus.ADVISOR_ASSIGNED, "student_clubs")
        execute_transaction([
    ...
    @classmethod
    def add_member(cls, org_id: str, club_id: str, student_id: str, role: str = "MEMBER") -> Dict[str, Any]:
        member_id = str(uuid4())
        execute_transaction([
            (
    ...
    @classmethod
    def list_clubs(cls, org_id: str, status: Optional[str] = None) -> List[Dict[str, Any]]:
        if status:
            rows = execute_query(
    ...
    @classmethod
    def _validate_transition(cls, org_id: str, club_id: str, target: ClubStatus, table: str) -> None:
        rows = execute_query(f"SELECT status FROM {table} WHERE id = %s AND org_id = %s",
                             (club_id, org_id), tenant_id=org_id)
        if not rows:
...
class EventService:
    ...
    @classmethod
    def propose_event(cls, org_id: str, data: Dict[str, Any], actor_id: str) -> Dict[str, Any]:
        event_id = str(uuid4())
        now = datetime.now(timezone.utc)
        execute_transaction([
            (
                ...
                """,
                (event_id, org_id, data.get("club_id"), data["title"],
                 data.get("description", ""), data.get("event_date"),
                 data.get("venue_requested", ""), data.get("expected_participants", 0),
                 data.get("budget_requested", 0),
                 EventStatus.PROPOSED.value, actor_id, now),
            )
        ], tenant_id=org_id)
    ...
    @classmethod
    def allot_venue(cls, org_id: str, event_id: str, venue: str, actor_id: str) -> Dict[str, Any]:
        ...
        if event_date and venue:
            conflicts = execute_query(
                ...
                (venue, event_date, org_id), tenant_id=org_id,
            )
            if conflicts:
                raise ValueError(f"Venue '{venue}' has a timetable conflict on {event_date}")

        cls._advance(org_id, event_id, EventStatus.VENUE_ALLOTTED, actor_id)
        execute_transaction([
            ("UPDATE club_events SET venue_allotted = %s WHERE id = %s AND org_id = %s",
             (venue, event_id, org_id))
        ], tenant_id=org_id)
        return {"id": event_id, "status": EventStatus.VENUE_ALLOTTED.value, "venue": venue}

    @classmethod
    def complete_event(cls, org_id: str, event_id: str, actual_participants: int, actor_id: str) -> Dict[str, Any]:
        cls._advance(org_id, event_id, EventStatus.COMPLETED, actor_id)
        execute_transaction([
    ...
    @classmethod
    def record_participation(cls, org_id: str, event_id: str, student_id: str, certificate: bool = False) -> Dict[str, Any]:
        part_id = str(uuid4())
        execute_transaction([
            (
    ...
    @classmethod
    def generate_noc(cls, org_id: str, student_id: str, competition_name: str, dates: str, actor_id: str) -> Dict[str, Any]:
    ...
    @classmethod
    def get_annual_activity_report(cls, org_id: str, academic_year: str) -> Dict[str, Any]:
        ...
        events = execute_query(
            ...
            (org_id, academic_year[:4]), tenant_id=org_id)
    ...
    @classmethod
    def _advance(cls, org_id: str, event_id: str, target: EventStatus, actor_id: str) -> None:
        rows = execute_query("SELECT status FROM club_events WHERE id = %s AND org_id = %s",
                             (event_id, org_id), tenant_id=org_id)
        if not rows:
    ...
    def _get(cls, org_id: str, event_id: str) -> Dict[str, Any]:
        rows = execute_query("SELECT * FROM club_events WHERE id = %s AND org_id = %s",
                             (event_id, org_id), tenant_id=org_id)
        if not rows:
            raise ValueError(f"Event {event_id} not found")
        return dict(rows[0])
