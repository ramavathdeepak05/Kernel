...
class GrievanceAnomalyDetector:
    ...
    def evaluate(
        ...
        window_hours = int(policy_engine.get_value(
            "grievance.spike_window_hours", org_id, 24
        ))
        spike_multiplier = float(policy_engine.get_value(
            "grievance.spike_multiplier", org_id, 3.0
        ))
        spike_min_count = int(policy_engine.get_value(
            "grievance.spike_min_count", org_id, 5
        ))
        invigilator_blackout_hours = int(policy_engine.get_value(
            "grievance.invigilator_blackout_hours", org_id, 48
        ))
        ...
        if anomaly_detected:
            ...
            execute_transaction([
                ...
                """, (
                    log_id, org_id, window_start, now,
                    current_count, round(spike_threshold, 1),
                )),
            ...
            logger.warning(
                ...
                org_id, respondent_id, current_count, spike_threshold,
            )
