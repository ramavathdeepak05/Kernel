...
class GuardianPortalService:
    ...
    @classmethod
    def provision_guardian(cls, org_id: str, student_id: str, guardian_phone: str,
                          guardian_name: str = "", relationship: str = "PARENT") -> Dict[str, Any]:
        ...
        existing = execute_query(
            ...
            (guardian_phone, org_id), tenant_id=org_id,
        )
        if existing:
    ...
    def send_otp(cls, org_id: str, phone: str) -> Dict[str, Any]:
        ...
        links = execute_query(
            ...
            (guardian_id, org_id), tenant_id=org_id,
        )
        student_id = str(links[0]["student_id"]) if links else ""

        otp = GuardianOTPManager.generate(phone, student_id, org_id)

        # In production: send via SMS/WhatsApp
        logger.info("Guardian OTP generated for phone=%s (guardian_id=%s)", phone[-4:], guardian_id)

        return {"message": "OTP sent to registered phone number."}

    @classmethod
    def verify_otp(cls, org_id: str, phone: str, otp: str) -> Optional[Dict[str, Any]]:
    ...
    def get_student_dashboard(cls, org_id: str, student_id: str) -> Dict[str, Any]:
        ...
        if att and att[0]["total"] > 0:
            dashboard["attendance_percentage"] = round(att[0]["present"] / att[0]["total"] * 100, 1)
        else:
        ...
        if dues:
            ...
            dashboard["last_payment"] = str(dues[0].get("last_payment_date", "")) if dues[0].get("last_payment_date") else None
        else:
