...
def _compute_grade(
    marks: float, max_marks: int, pass_marks: int, grade_table: list
...
class GradeService:
    ...
    def enter_grades(cls, org_id: str, req: GradesBulkEntry, actor_id: str) -> dict:
        ...
        AuditLog.log(
            ...
            actor_type="human",
            entity_type="grades",
            entity_id=req.exam_schedule_id,
            org_id=org_id,
            module="E06-S03",
            metadata={"entered": entered, "academic_year": req.academic_year},
        )
    ...
    def compute_semester_result(
        ...
        if not grades:
            raise BusinessRuleViolation(
                message=f"No grades found for student {student_id} sem {semester}"
            )

        total_credits = sum(int(g["credits"]) for g in grades)
        earned_credits = sum(
            int(g["credits"]) for g in grades if g["is_pass"] and not g["is_absent"]
        )
        weighted_pts = sum(
        ...
        AuditLog.log(
            ...
            actor_id=actor_id,
            actor_role="system",
            entity_type="compute_semester_result",
            entity_id="",
            tenant_id=org_id,
            metadata={"source": "compute_semester_result"},
        )
    ...
    def publish_results(
        ...
        logger.info(
            "Results published: org=%s year=%s sem=%d", org_id, academic_year, semester
        )
        return {
            "status": "published",
            "academic_year": academic_year,
