...
class TranscriptService:
    @classmethod
    def generate(
        cls, org_id: str, student_id: str, actor_id: str, is_official: bool = False
    ) -> dict:
        """
        ...
        logger.info(
            "Transcript generated: student=%s hash=%s", student_id, content_hash[:8]
        )
        return {
            "id": tid,
            "student_id": student_id,
    ...
    @classmethod
    def _build_pdf(
        cls, student: dict, results, grade_rows, org_id: str, is_official: bool
    ) -> bytes:
        buffer = BytesIO()
        ...
        elements.append(
            Paragraph(f"Roll No: {student['roll_number']}", styles["Normal"])
        )
        ...
        for (yr, sem), sem_grades in groupby(
            grade_list, key=lambda x: (x["academic_year"], x["semester"])
        ):
            ...
            for g in sem_grades:
                marks = (
                    f"{g['marks_obtained']:.1f}/{g['max_marks']}"
                    if not g["is_absent"]
                    else "ABSENT"
                )
                table_data.append(
        ...
        if results:
            ...
            elements.append(
                Paragraph(
                    f"Cumulative GPA (CGPA): {last.get('cgpa', '-')}",
                    styles["Heading2"],
                )
            )
