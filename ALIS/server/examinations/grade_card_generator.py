...
class GradeCardGeneratorService:
    ...
    async def get_batch_status(
        ...
        if not rows:
            return {"total": 0, "ready": 0, "failed": 0, "in_progress": 0, "pct_complete": 0}
...
def pre_generate_grade_cards(self, batch_id: str, semester_id: str, org_id: str, actor_id: str):
    ...
    """
    logger.info("Starting grade card pre-generation: semester=%s org=%s", semester_id, org_id)
    ...
    if students:
        ...
        for student in students:
            ...
            """, (
                cache_id, org_id, semester_id, student_id, batch_id, minio_path,
            )))
    ...
    AuditLog.log(
        ...
        detail={"semester_id": semester_id, "total": total, "generated": generated, "failed": failed},
    )
...
def _generate_single_card(
    ...
    # Compute SGPA
    total_gp = sum(
        float(r.get("grade_points", 0)) * float(r.get("credits", 0))
        for r in results
    )
