...
@celery_app.task(name="admissions.advance_pipeline", bind=True, max_retries=3, default_retry_delay=30)
def advance_pipeline(self, org_id: str, applicant_id: str, actor_id: str = "system") -> dict:
    """
    ...
    try:
        ...
        logger.info(
            ...
            applicant_id, result.status, result.next_step,
        )
        return {
            "applicant_id": applicant_id,
...
@celery_app.task(name="admissions.process_intake", bind=True, max_retries=3, default_retry_delay=15)
def process_intake(self, org_id: str, payload: dict) -> dict:
    ...
    try:
        ...
        try:
            ...
            logger.warning("process_intake: dedup check failed (non-fatal): %s", dedup_exc)

        # 3. Trigger automation pipeline
        advance_pipeline.delay(org_id=org_id, applicant_id=applicant.id)

        logger.info("process_intake: created applicant %s for org %s", applicant.id, org_id)
...
def check_fee_overdue() -> dict:
    ...
    try:
        ...
        flagged = 0
        for row in rows:
            validity_days = int(PolicyStore.get(row["org_id"], PolicyKey.OFFER_VALIDITY_DAYS) or 30)
            ...
            if date.today() > expiry:
                DomainEventBus.publish(DomainEvent(
                    ...
                    org_id=row["org_id"],
                    payload={"issued_at": str(row["issued_at"]), "expired_on": str(expiry)},
                    actor_id="system",
                ))
                flagged += 1
