"""
ALIS Research Module — Agent Registry

MODULE: M8 — Research
LAYER: Layer 2 (Agentic Decisions)

Registered Agents:
    plagiarism_advisor_v1 — PhD thesis plagiarism remediation advice
"""
...
class ResearchAgentRegistry:
    """Module-scoped agent registry for M8 — Research."""

    _agents: Dict[str, AgentMeta] = {
        "plagiarism_advisor_v1": AgentMeta(
            agent_id="plagiarism_advisor_v1",
            module="M8",
            model_version="qwen2.5:1.5b-instruct-q8_0",
            prompt_version="v1",
            invocation_class="EVALUATIVE",
            authorization_policy="phd_supervisor_or_research_coordinator_or_system",
            allowed_tools=(
                "tool.rag.retriever",
                "tool.policy.lookup",
            ),
            description=(
                "Advises PhD scholars on how to remediate plagiarism findings from external detection "
                "tools (iThenticate/Turnitin). Given the similarity report summary, identifies which "
                "matched sections require paraphrasing, proper citation, or removal, and suggests an "
                "action plan within institutional policy thresholds. Returns advice as a Draft for "
                "supervisor confirmation — does not modify the thesis document."
            ),
        ),
    }
    ...
    def _ensure_executors_loaded(cls):
        if not cls._executors:
            from server.agents.research.plagiarism_advisor_v1 import execute_plagiarism_advisor
            cls._executors["plagiarism_advisor_v1"] = execute_plagiarism_advisor
    ...
    def execute(cls, agent_name: str, context: AIGatewayContext,
        ...
        if agent_name not in cls._agents:
            raise ValueError(f"Agent '{agent_name}' not registered in M8 (Research).")
        ...
        AuditLog.log(action=AuditAction.AGENT_EXECUTION, actor_id=context.actor_id,
                     ...
                     metadata={"phase": "start", "module": "M8", "request_id": context.request_id})
