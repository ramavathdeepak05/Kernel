...
class ExaminationsAgentRegistry:
    ...
    def _ensure_executors_loaded(cls):
        if not cls._executors:
            from server.agents.examinations.result_analyzer_v1 import execute_result_analyzer
    ...
    def execute(cls, agent_name: str, context: AIGatewayContext,
        ...
        if agent_name not in cls._agents:
            raise ValueError(f"Agent '{agent_name}' not registered in M3 (Examinations).")
        ...
        AuditLog.log(action=AuditAction.AGENT_EXECUTION, actor_id=context.actor_id,
                     ...
                     metadata={"phase": "start", "module": "M3", "request_id": context.request_id})
