...
class StudentServicesAgentRegistry:
    ...
    def _ensure_executors_loaded(cls):
        if not cls._executors:
            from server.agents.student_services.grievance_classifier_v1 import (
                execute_grievance_classifier,
            )
    ...
    def execute(
        ...
        if agent_name not in cls._agents:
            raise ValueError(
                f"Agent '{agent_name}' not registered in M6 (Student Services)."
            )
