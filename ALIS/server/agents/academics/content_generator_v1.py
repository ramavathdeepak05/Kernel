...
from server.core.ai_gateway import AIInvocationResult
...
def _validate_input(input_data: Dict[str, Any]) -> None:
    required = ["course_name", "course_code", "co_codes", "co_descriptions",
                "bloom_levels", "syllabus_topics"]
    missing = [k for k in required if k not in input_data]
    if missing:
        raise ValueError(f"content_generator: missing required input_data keys: {missing}")
    ...
    if len(co_codes) != len(co_desc) or len(co_codes) != len(bloom):
        raise ValueError("co_codes, co_descriptions, and bloom_levels must have equal length")
    if not input_data["syllabus_topics"]:
        raise ValueError("syllabus_topics must be a non-empty list")


def _build_variables(input_data: Dict[str, Any], generation_type: str) -> Dict[str, Any]:
    ...
    if generation_type == "lecture_notes":
        base["lecture_duration_minutes"] = input_data.get("lecture_duration_minutes", 60)
