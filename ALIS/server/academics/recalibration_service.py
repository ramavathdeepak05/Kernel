...
class RecalibrationRequest(BaseModel):
    ...
    scope_ref: str | None = None  # department_id / program_id / section_id
    reason: str
...
class RecalibrationService:
    ...
    async def trigger(
        ...
        affected_sessions_rows = execute_query(
            f"""  # noqa: S608
        ...
        # ---------------------------------------------------------------
        shiftable_deadlines_rows = execute_query(
            f"""  # noqa: S608
        ...
        locked_exam_rows = execute_query(
            f"""  # noqa: S608
            SELECT d.id, d.title, d.due_date
            FROM course_deadlines d
            WHERE d.org_id = %s
