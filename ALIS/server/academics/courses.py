...
class CourseService:
    ...
    def create(cls, org_id: str, req: CourseCreate, actor_id: str) -> dict:
        ...
        if existing:
            raise BusinessRuleViolation(message=f"Course code '{req.code}' already exists")
        ...
        execute_transaction([(
            ...
            """,
            (cid, org_id, req.program_id, req.name, req.code, req.semester,
             req.credits, req.course_type.value, json.dumps(req.metadata)),
        )])

        AuditLog.log(action=AuditAction.CREATE, actor_id=actor_id, actor_type="human",
                     entity_type="course", entity_id=cid, org_id=org_id, module="E05-S02",
    ...
    @classmethod
    def list(cls, org_id: str, program_id: str | None = None, semester: int | None = None) -> list[dict]:
