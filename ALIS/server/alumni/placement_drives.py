...
class PlacementDriveService:
    ...
    def create(cls, org_id: str, req: DriveCreate, actor_id: str) -> dict:
        ...
        execute_transaction([(
            ...
            """,
            (
                did, org_id, req.company_name, req.jd_title,
                req.jd_description, req.jd_skills or [],
                req.ctc_min_lpa, req.ctc_max_lpa, req.drive_type,
                req.eligibility_min_cgpa, req.eligibility_max_backlogs,
                req.eligible_programs or [], req.eligible_batches or [],
                req.registration_open_at, req.registration_close_at,
                req.drive_date, req.venue, req.apply_one_offer_policy,
                actor_id,
            ),
        )])
        ...
        AuditLog.log(
            ...
            resource_type="placement_drive", resource_id=did,
    ...
    def list(
        ...
        if drive_type:
            sql += " AND drive_type = %s"; params.append(drive_type)
    ...
    def record_offer_accepted(
        ...
        DomainEventBus.publish(DomainEvent(
            ...
            org_id=org_id,
            payload={"student_id": student_id, "drive_id": drive_id, "ctc_lpa": ctc_lpa},
        ))
...
class OfferRevocationService:
    ...
    def submit_revocation(
        ...
        execute_transaction([(
            ...
            """,
            (
                rid, org_id, req.student_id, req.drive_id, req.company_name,
                req.revocation_reason, req.revocation_date,
                req.evidence_url, actor_id,
            ),
        )])

        logger.info(
            "Offer revocation submitted: id=%s student=%s company=%s",
            rid, req.student_id, req.company_name,
        )
    ...
    def tpo_verify(
        ...
        if approved:
            ...
            logger.info(
                ...
                rev["student_id"], rev["company_name"],
            )
        else:
            execute_transaction([(
                """UPDATE placement_offer_revocations
...
class VerbalLockService:
    ...
    def create_lock(
        ...
        execute_transaction([
            (
                ...
                """,
                (
                    lid, org_id, req.student_id, req.drive_id, req.company_name,
                    actor_id, req.company_contact_name, req.company_contact_email,
        ...
        logger.info(
            ...
            lid, req.student_id, req.company_name, req.formal_offer_due_by,
        )
    ...
    def expire_overdue_locks(cls, org_id: str) -> int:
        ...
        ops = [
            (
                f"UPDATE placement_offer_locks SET status = 'AUTO_EXPIRED', unlocked_at = NOW() "
