...
class AlumniTransitionSaga:
    ...
    @classmethod
    def record_signal(cls, org_id: str, student_id: str, signal: str, actor_id: str = "system") -> Dict[str, Any]:
        ...
        # Check if all 3 signals are present
        ...
        all_required = {SagaSignal.RESULT_PUBLISHED.value, SagaSignal.DUES_CLEARED.value, SagaSignal.GRADUATION_VERIFIED.value}

        if received >= all_required:
            # All signals received — complete the saga
            return cls._complete_transition(org_id, student_id, actor_id)
    ...
    @classmethod
    def _complete_transition(cls, org_id: str, student_id: str, actor_id: str) -> Dict[str, Any]:
        """All 3 signals received — transition student to alumni."""
        now = datetime.now(timezone.utc)
        ...
        if not student:
            logger.error("AlumniTransitionSaga: student %s not found", student_id)
            return {"student_id": student_id, "saga_complete": False, "error": "Student not found"}
        ...
        execute_transaction([
            ...
            (
                ...
                """,
                (alumni_id, org_id, student_id, s.get("display_name", s.get("username", "")),
                 s.get("email", ""), now),
            ),
            # Mark saga complete
            (
        ...
        # Seal transcript (permanent, immutable)
        try:
            ...
            logger.warning("Transcript seal failed (non-fatal): %s", exc)

        # Publish alumni.created event
        DomainEventBus.publish(DomainEvent(
            event_type="alumni.created",
            org_id=org_id,
            payload={"alumni_id": alumni_id, "student_id": student_id},
            actor_id=actor_id,
        ))

        AuditLog.log(
            action=AuditAction.CREATE, actor_id=actor_id, actor_role="system",
            entity_type="alumni_profile", entity_id=alumni_id, tenant_id=org_id,
            metadata={"student_id": student_id, "event": "alumni_transition_saga_complete"},
        )
...
class GraduationEmploymentDeclaration:
    ...
    @classmethod
    def submit(cls, org_id: str, student_id: str, data: Dict[str, Any], actor_id: str) -> Dict[str, Any]:
        ...
        for field in required:
            if not data.get(field):
                raise ValueError(f"Field '{field}' is required for category {category.value}")
        ...
        execute_transaction([
            (
                ...
                """,
                (
                    decl_id, org_id, student_id, category.value,
                    data.get("employer_name", ""), data.get("employer_cin", ""),
                    data.get("institution", ""), data.get("program", ""),
                    data.get("startup_name", ""), data.get("gstin", ""),
                    data.get("business_name", ""), now,
                ),
            )
        ], tenant_id=org_id)
        ...
        AuditLog.log(
            ...
            entity_type="graduation_employment_declaration", entity_id=decl_id, tenant_id=org_id,
    ...
    def get_nirf_go_data(cls, org_id: str, graduation_year: str) -> Dict[str, Any]:
        ...
        rows = execute_query(
            ...
            GROUP BY category
            """,
            (org_id, int(graduation_year)), tenant_id=org_id,
        )
        ...
        entrepreneur = categories.get("ENTREPRENEURSHIP_OWN", 0) + categories.get("ENTREPRENEURSHIP_FAMILY", 0)
        ...
        return {
            ...
            "go_rate": round((placed + higher + entrepreneur) / total * 100, 1) if total > 0 else 0,
