...
APPRAISAL_TRANSITIONS = {
    ...
    AppraisalStatus.HOD_REVIEW: {AppraisalStatus.HOD_ENDORSED, AppraisalStatus.HOD_RETURNED},
    AppraisalStatus.HOD_RETURNED: {AppraisalStatus.SELF_SUBMITTED},
    AppraisalStatus.HOD_ENDORSED: {AppraisalStatus.DEAN_REVIEW},
    AppraisalStatus.DEAN_REVIEW: {AppraisalStatus.DEAN_ENDORSED, AppraisalStatus.DEAN_RETURNED},
    AppraisalStatus.DEAN_RETURNED: {AppraisalStatus.HOD_REVIEW},
    AppraisalStatus.DEAN_ENDORSED: {AppraisalStatus.COMMITTEE_REVIEW},
    AppraisalStatus.COMMITTEE_REVIEW: {AppraisalStatus.PROMOTED, AppraisalStatus.DEFERRED, AppraisalStatus.REJECTED},
...
class AppraisalService:
    ...
    @classmethod
    def create(cls, org_id: str, staff_id: str, data: Dict[str, Any], actor_id: str) -> Dict[str, Any]:
        ...
        execute_transaction([
            (
                ...
                (
                    ...
                    AppraisalStatus.DRAFT.value, now,
                ),
            )
        ], tenant_id=org_id)
    ...
    @classmethod
    def advance(cls, org_id: str, appraisal_id: str, target: str, actor_id: str,
                review_comments: str = "", actor_role: str = "system") -> Dict[str, Any]:
        """Move appraisal to next stage (validates transition)."""
        rows = execute_query(
            "SELECT status, staff_id FROM appraisals WHERE id = %s AND org_id = %s",
            (appraisal_id, org_id), tenant_id=org_id,
        )
        if not rows:
        ...
        elif target_status in (AppraisalStatus.PROMOTED, AppraisalStatus.DEFERRED, AppraisalStatus.REJECTED):
        ...
        execute_transaction([
            (f"UPDATE appraisals SET {set_clause}, updated_at = NOW() WHERE id = %s AND org_id = %s",
             tuple(values))
        ], tenant_id=org_id)
        ...
        AuditLog.log(
            action=AuditAction.UPDATE, actor_id=actor_id, actor_role=actor_role,
            entity_type="appraisal", entity_id=appraisal_id, tenant_id=org_id,
            metadata={"from": current.value, "to": target_status.value, "comments": review_comments},
        )
    ...
    @classmethod
    def list_pending(cls, org_id: str, stage: Optional[str] = None) -> List[Dict[str, Any]]:
        if stage:
            rows = execute_query(
