...
class PublicationDiscoveryService:
    ...
    @classmethod
    async def discover_from_orcid(cls, org_id: str, staff_id: str, orcid_id: str) -> List[Dict[str, Any]]:
                ...
                for group in groups:
                    ...
                    execute_transaction([
                        (
                            ...
                            """,
                            (pub_id, org_id, staff_id, title, journal, pub_year, doi,
    ...
    async def discover_from_scopus(
        cls, org_id: str, staff_id: str, author_name: str, scopus_api_key: str,
    ) -> List[Dict[str, Any]]:
                ...
                for entry in results:
                    ...
                    execute_transaction([
                        (
                            ...
                            """,
                            (pub_id, org_id, staff_id, title, journal, year, doi,
                             citations, PublicationStatus.UNVERIFIED.value),
                        )
                    ], tenant_id=org_id)

                    discovered.append({
                        "id": pub_id, "title": title, "journal": journal,
                        "year": year, "doi": doi, "citations": citations,
                        "source": "SCOPUS", "status": PublicationStatus.UNVERIFIED.value,
                    })
    ...
    @classmethod
    def verify_publication(cls, org_id: str, pub_id: str, decision: str, actor_id: str) -> Dict[str, Any]:
        ...
        execute_transaction([
            ("UPDATE faculty_publications SET status = %s, verified_by = %s, verified_at = NOW() "
             ...
             (decision, actor_id, pub_id, org_id, PublicationStatus.UNVERIFIED.value))
        ], tenant_id=org_id)

        AuditLog.log(
            action=AuditAction.UPDATE, actor_id=actor_id, actor_role="faculty",
            entity_type="faculty_publication", entity_id=pub_id, tenant_id=org_id,
            metadata={"decision": decision, "event": "publication_verification"},
        )
        return {"id": pub_id, "status": decision}

    @classmethod
    def get_faculty_publications(cls, org_id: str, staff_id: str, status: Optional[str] = None) -> List[Dict[str, Any]]:
        if status:
            rows = execute_query(
                "SELECT * FROM faculty_publications WHERE staff_id = %s AND org_id = %s AND status = %s "
                "ORDER BY year DESC, discovered_at DESC",
                (staff_id, org_id, status), tenant_id=org_id,
            )
        else:
            rows = execute_query(
    ...
    async def run_weekly_scan(cls, org_id: str) -> Dict[str, Any]:
        ...
        for s in staff:
            ...
            if pubs:
                logger.info("Discovered %d new publications for %s", len(pubs), s["name"])

        return {"staff_scanned": len(staff), "publications_discovered": total_discovered}
