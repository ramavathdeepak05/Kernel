    ...
    def create(cls, org_id: str, req: LeaveTypeCreate, actor_id: str) -> dict:
        ...
        if existing:
            raise BusinessRuleViolation(
                message=f"Leave type code {req.code} already exists"
            )
...
class LeaveService:
    @classmethod
    def apply(
        cls, org_id: str, staff_id: str, req: LeaveRequestCreate, actor_id: str
    ) -> dict:
        # Validate staff
        ...
        used = cls._used_days(
            org_id, staff_id, req.leave_type_id, req.from_date[:4]
        )  # current year
        ...
        if overlap:
            raise BusinessRuleViolation(
                message="Overlapping approved leave exists for this period"
            )
    ...
    @classmethod
    def decide(
        cls, org_id: str, request_id: str, decision: LeaveDecision, actor_id: str
    ) -> dict:
        rows = execute_query(
        ...
        if decision.decision == "APPROVED":
            ...
            leave_ops = cls._build_attendance_leave_queries(
                org_id,
                str(req["staff_id"]),
                str(req["from_date"]),
                str(req["to_date"]),
    ...
    @classmethod
    def _used_days(
        cls, org_id: str, staff_id: str, leave_type_id: str, year: str
    ) -> float:
        rows = execute_query(
            """
    ...
    @classmethod
    def _build_attendance_leave_queries(
        cls, org_id: str, staff_id: str, from_date: str, to_date: str, actor_id: str
    ) -> list[tuple]:
        ...
        try:
            ...
            logger.warning(
                "LeaveService: attendance marking failed (non-fatal): %s", exc
            )
        return ops
