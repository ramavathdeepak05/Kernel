...
class ProcessDefinitionService:
    ...
    @staticmethod
    def _upsert_steps(org_id: str, process_id: str, steps: List[Dict[str, Any]]) -> None:
        import json
        ops = []
    ...
    @staticmethod
    def get(org_id: str, process_id: str) -> Optional[Dict[str, Any]]:
        rows = execute_query(
    ...
    @staticmethod
    def get_steps(org_id: str, process_id: str) -> List[Dict[str, Any]]:
        rows = execute_query(
    ...
    @staticmethod
    def list(org_id: str, active_only: bool = True, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
        where = "WHERE org_id = %s"
        params: list = [org_id]
        if active_only:
            where += " AND is_active = TRUE"
        rows = execute_query(
            f"SELECT * FROM process_definitions {where} ORDER BY created_at DESC LIMIT %s OFFSET %s",
            params + [limit, offset],
        )
        return [dict(r) for r in rows]

    @staticmethod
    def update(org_id: str, process_id: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        sets, params = [], []
        for col in ("name", "description", "trigger_event", "trigger_filter", "is_active"):
        ...
        execute_transaction([(
            f"UPDATE process_definitions SET {', '.join(sets)} WHERE id = %s AND org_id = %s",
            params,
        )])
        return ProcessDefinitionService.get(org_id, process_id)
    ...
    @staticmethod
    def find_by_event(event_name: str) -> List[Dict[str, Any]]:
