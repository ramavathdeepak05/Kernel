    ...
    try:
        ...
        for defn in definitions:
            ...
            try:
                ...
                logger.info(
                    "Launched process '%s' for event '%s'", defn["name"], event_name
                )
            except Exception as exc:
                logger.exception(
                    "Failed to launch process '%s' for event '%s': %s",
                    defn["name"],
                    event_name,
                    exc,
                )
        ...
        logger.exception(
            "_launch_matching_processes error for '%s': %s", event_name, exc
        )
...
def register_all() -> None:
    ...
    logger.info(
        "E13 Process Engine event handlers registered (%d events)", len(_TRIGGER_EVENTS)
    )
