...
class ReportingGateService:
    ...
    def enter_reporting_pending(
        ...
        sla_days = int(policy_engine.get_value(
            "admissions.reporting_gate_sla_days", org_id, 5
        ))
        ...
        AuditLog.log(
            ...
            detail={"status": STATE_REPORTING_PENDING, "deadline": deadline.isoformat()},
        )
        ...
        return {"status": STATE_REPORTING_PENDING, "reporting_deadline": deadline.isoformat()}
    ...
    def report_in(
        ...
        if feature_flags.is_enabled("admissions.biometric_reporting_gate", org_id):
            if method == "BIOMETRIC" and not biometric_verified:
                raise BusinessRuleViolation("Biometric verification required and not confirmed")
    ...
    def check_reporting_deadlines(cls) -> None:
        ...
        for org_row in orgs:
            ...
            r1 = int(policy_engine.get_value("admissions.reporting_gate_reminder_1", org_id, 1))
            r2 = int(policy_engine.get_value("admissions.reporting_gate_reminder_2", org_id, 2))
            r3 = int(policy_engine.get_value("admissions.reporting_gate_reminder_3", org_id, 3))
            ...
            for app in applicants:
                ...
                if now >= deadline:
                    # Forfeit seat
                    cls._forfeit_seat(org_id, app_id, str(app.get("category", "GENERAL")), str(app.get("quota", "GENERAL")))
                elif days_elapsed >= r3:
                    cls._send_reminder(org_id, app_id, "DAY_3_FORFEITURE_WARNING", deadline)
                elif days_elapsed >= r2:
                    cls._send_reminder(org_id, app_id, "DAY_2_PARENT_REMINDER", deadline)
                elif days_elapsed >= r1:
                    cls._send_reminder(org_id, app_id, "DAY_1_REMINDER", deadline)

    @classmethod
    def _forfeit_seat(cls, org_id: str, application_id: str, category: str, quota: str) -> None:
        execute_transaction([("""
            UPDATE applicants
    ...
    @classmethod
    def _send_reminder(cls, org_id: str, application_id: str, template_key: str, deadline: datetime) -> None:
        DomainEventBus.publish(DomainEvent(
