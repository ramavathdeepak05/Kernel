...
| (c) | Take all **Art.32 security measures** | Point to: schema-per-tenant + RLS isolation, HMAC-peppered API keys, encryption in transit/at rest, the K·02 tamper-evident ledger, fail-closed governance, and a CI supply-chain gate (lint+tests blocking; Trivy image scan + pip-audit + CycloneDX SBOM in report mode — image signing not yet). (Attach the SECURITY.md / trust-center summary.) |
